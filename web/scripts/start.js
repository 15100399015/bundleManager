...
new ResolveCommand({
    commandls: [
        { command: "production", options: ["analyse", "publish"] },
    ...
    if (envModel === "development") {
        ...
        config.enablePublish = _rc.hasOption("publish")
    }
 ...
 * @param {*} config
 */
function webpackServe(compiler, webpackConfig, envMode, config) {
 ...
 * @param {*} config
 */
function webpackBuild(compiler, webpackConfig, envMode, config) {
    compiler.run((err, stats) => {
        ...
        if (isSuccessful) {
            ...
            if (config.enablePublish) {
                publishToServer(statsData.outputPath)
            }
        } else {
            if (stats.hasErrors()) {
...
function publishToServer(outputPath, servePublicPath = "../server/public") {
    Logger.logInfo("Start Publish to Server");
    fs.cpSync(outputPath, path.resolve(cwd(), servePublicPath), { recursive: true })
